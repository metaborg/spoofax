...
public class LanguageSourcesPrimitive extends AbstractPrimitive {
    ...
    @Inject public LanguageSourcesPrimitive(IResourceService resourceService,
    ...
    public boolean call(IContext env, Strategy[] svars, IStrategoTerm[] tvars)
        ...
        final Iterable<FileObject> sources =
                languagePathService.sources(project, languageName);
        final List<IStrategoTerm> terms = Lists.newArrayList();
        for ( FileObject source : sources ) {
            File localFile = resourceService.localFile(source);
