...
public class UpgradeSpoofaxProjectWizard extends Wizard {
    ...
    private void doFinish(IProgressMonitor monitor) throws Exception {
        monitor.beginTask("Upgrading Spoofax Project", 6);

        UpgradeMainXML(monitor);
        monitor.worked(1);
        UpgradeGeneratedXML(monitor);
        monitor.worked(1);
    ...
    private void UpgradeMainXML(IProgressMonitor monitor) throws Exception {
        ...
        final Attr buildPropertyAttr =
            (Attr) xpath.evaluate("//project/property[@name='build']/@location", document,
                XPathConstants.NODE);
        ...
        final Attr syntaxPropertyAttr =
            (Attr) xpath.evaluate("//project/property[@name='syntax']/@location", document,
                XPathConstants.NODE);
        ...
        // New src-gen modules
        final Node ppModuleNode =
            (Node) xpath.evaluate("//project/property[@name='ppmodule']", document, XPathConstants.NODE);
        final Node sigModuleNode =
            (Node) xpath.evaluate("//project/property[@name='sigmodule']", document, XPathConstants.NODE);

        final Node metasdfmodule =
            (Node) xpath.evaluate("//project/property[@name='metasdfmodule']", document, XPathConstants.NODE);
        ...
        final Node sdfSrcGenProperty =
            (Node) xpath.evaluate("//project/property[@name='sdf-src-gen']", document, XPathConstants.NODE);
        final Node ppProperty =
            (Node) xpath.evaluate("//project/property[@name='pp']", document, XPathConstants.NODE);
        final Node signaturesProperty =
            (Node) xpath.evaluate("//project/property[@name='signatures']", document, XPathConstants.NODE);
        final Node libGenProperty =
            (Node) xpath.evaluate("//project/property[@name='lib-gen']", document, XPathConstants.NODE);
        final Node syntaxRelativeProperty =
            (Node) xpath.evaluate("//project/property[@name='syntax.rel']", document, XPathConstants.NODE);
        final Node transRelativeProperty =
            (Node) xpath.evaluate("//project/property[@name='trans.rel']", document, XPathConstants.NODE);
        final Node includeRelativeProperty =
            (Node) xpath.evaluate("//project/property[@name='include.rel']", document, XPathConstants.NODE);
        final Node libGenRelativeProperty =
            (Node) xpath.evaluate("//project/property[@name='lib-gen.rel']", document, XPathConstants.NODE);

        final Node buildPropertyNode =
            (Node) xpath.evaluate("//project/property[@name='build']", document, XPathConstants.NODE);
        ...
        final Node nativePathNode =
            (Node) xpath.evaluate("//project/property[@name='nativepath']", document, XPathConstants.NODE);

        final Node environmentConfigCommentNode =
            (Node) xpath.evaluate(
                "//project/comment()[.=' Environment configuration for command-line builds ']", document,
                XPathConstants.NODE);
        ...
        if(nativePathNode == null) {
            ...
            projectNode.insertBefore(nativePathElem, environmentConfigCommentNode.getNextSibling()
                .getNextSibling());
            changed = true;
        }
        ...
        final Node generateSourcesNode =
            (Node) xpath
                .evaluate("//project/target[@name='generate-sources']", document, XPathConstants.NODE);
        final Node packageNode =
            (Node) xpath.evaluate("//project/target[@name='package']", document, XPathConstants.NODE);
    ...
    private void UpgradeGeneratedXML(IProgressMonitor monitor) throws Exception {
        final IFile file = (IFile) project.findMember("build.generated.xml");
        if(file == null)
            return;
        final Document document = OpenXML(file);
        final XPath xpath = XPathFactory.newInstance().newXPath();
        ...
        // Add dummy generate-sources-java, package-java, generate-sources-ctree, package-ctree targets, to
        // prevent target not found errors in the main build file.
        final Node genSourceJavaNode =
            (Node) xpath.evaluate("//project/target[@name='generate-sources-java']", document,
                XPathConstants.NODE);
        final Node pkgJavaNode =
            (Node) xpath.evaluate("//project/target[@name='package-java']", document, XPathConstants.NODE);
        final Node genSourceCtreeNode =
            (Node) xpath.evaluate("//project/target[@name='generate-sources-ctree']", document,
                XPathConstants.NODE);
        final Node pkgCtreeNode =
            (Node) xpath.evaluate("//project/target[@name='package-ctree']", document, XPathConstants.NODE);

        if(genSourceJavaNode == null) {
            final Element targetElem = document.createElement("target");
            targetElem.setAttribute("name", "generate-sources-java");
            projectNode.appendChild(targetElem);
            changed = true;
        }
        if(pkgJavaNode == null) {
            final Element targetElem = document.createElement("target");
            targetElem.setAttribute("name", "package-java");
            projectNode.appendChild(targetElem);
            changed = true;
        }
        if(genSourceCtreeNode == null) {
            final Element targetElem = document.createElement("target");
            targetElem.setAttribute("name", "generate-sources-ctree");
            projectNode.appendChild(targetElem);
            changed = true;
        }
        if(pkgCtreeNode == null) {
            final Element targetElem = document.createElement("target");
            targetElem.setAttribute("name", "package-ctree");
            projectNode.appendChild(targetElem);
            changed = true;
        }
    ...
    private void UpgradeClasspath(IProgressMonitor monitor) throws Exception {
        ...
        final Attr sourceExcludeAttr =
            (Attr) xpath.evaluate("//classpath/classpathentry[@kind='src']/@excluding", document,
                XPathConstants.NODE);
        ...
        final Attr outputPathAttr =
            (Attr) xpath.evaluate("//classpath/classpathentry[@kind='output']/@path", document,
                XPathConstants.NODE);
    ...
    private void UpgradeManifest(IProgressMonitor monitor) throws Exception {
        ...
        final Set<String> requiredDependencies =
            Sets.newHashSet("org.spoofax.interpreter.externaldeps", "org.spoofax.interpreter.core",
                "org.spoofax.terms", "org.strategoxt.strj", "org.spoofax.jsglr", "org.strategoxt.imp.runtime");
    ...
    private Document OpenXML(IFile file) throws SAXException, IOException, CoreException,
        ParserConfigurationException {
        return DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(file.getContents());
    }

    private void WriteXML(IFile file, Document document, IProgressMonitor monitor)
