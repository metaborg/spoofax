 ...
 */
@SuppressWarnings("restriction") public class DescriptorRegistry {
    private final EditorRegistry editorRegistry;
    private final EditorDescriptor spoofaxEditor;
    /**
     * A complete list of all dynamically loaded languages, of which we ensure they exist in the Eclipse editor
     * registry.
     */
    private final List<Language> asyncAddedLanguages = new ArrayList<Language>();


    @Inject public DescriptorRegistry() {
    ...
    private class AsyncEditorRegisterer implements Runnable {
        ...
        private void addMapping(List<IFileEditorMapping> mappings, String extension, Language language) {
            ...
            if(!existing)
                mapping = new DynamicEditorMapping(language, extension);
            // TODO: create something like a IMPFileEditorMapping, linking to our favorite image.
    ...
    private static class DynamicEditorMapping extends FileEditorMapping {
        private final ImageDescriptor imageDescriptor = UIPlugin.imageDescriptorFromPlugin(RuntimePlugin.getInstance()
            .getID(), Descriptor.DEFAULT_ICON);
