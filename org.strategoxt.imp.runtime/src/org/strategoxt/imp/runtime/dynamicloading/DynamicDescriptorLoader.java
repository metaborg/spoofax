...
/**
 * This class updates any editors in the environment, triggered by resource change events.
 */
public class DynamicDescriptorLoader implements IResourceChangeListener {
    private static final int SCHEDULE_DELAY = 300;

    /** Use a provider to break cyclic dependency with DynamicDescriptorBuilder. Provider returns a singleton. */
    private final Provider<DynamicDescriptorBuilder> builderProvider;
    private final Queue<IResourceChangeEvent> asyncEventQueue = new ArrayDeque<IResourceChangeEvent>();
    private final Set<String> asyncIgnoreOnce = Collections.synchronizedSet(new HashSet<String>());
    private final AstMessageHandler asyncMessageHandler = new AstMessageHandler(AstMessageHandler.ANALYSIS_MARKER_TYPE);

    private boolean isAsyncEventHandlerActive;


    @Inject public DynamicDescriptorLoader(Provider<DynamicDescriptorBuilder> builderProvider) {
        this.builderProvider = builderProvider;
    }


    /**
     * Loads the editor for the specified descriptor, and ignore it at the next resource event arrives.
     */
    ...
    private static boolean isSignificantChangeDeep(IResourceDelta delta) {
        class IsSignificantVisitor implements IResourceDeltaVisitor {
            ...
            public boolean visit(IResourceDelta delta) throws CoreException {
                if(FileNotificationServer.isSignificantChange(delta)
                    && isSignificantName(delta.getResource().getName())) {
                    result = true;
                }
                return true;
            }
        ...
        if(isSignificantName(name)) {
            if(name.endsWith(".packed.esv")) {
                ...
                if(!source.equals(resource) && source.exists() && !startup) {
                    ...
                    builderProvider.get().updateResource(source, monitor);
    ...
    public void loadPackedDescriptor(IResource descriptor) {
        ...
        try {
            ...
            reportError(descriptor, e.getOffendingTerm(), "Error in descriptor " + descriptor + ": " + e.getMessage(),
                e);
        } catch(IOException e) {
    ...
    public static IResource getSourceDescriptor(IResource packedDescriptor) {
        ...
        if(result == null) {
            Environment.logException("Source descriptor not found", new FileNotFoundException("include/" + name
                + ".packed.esv"));
            return packedDescriptor;
        } else {
            return result;
