...
public class StrategoBuilderListener implements IModelListener {
	...
	private StrategoBuilderListener(UniversalEditor editor, IEditorPart targetEditor, IFile targetFile,
			IBuilder builder, ISimpleTerm selection) {
	...
	public static StrategoBuilderListener addListener(UniversalEditor editor, IEditorPart target, IFile file, IBuilder builder, ISimpleTerm node) {
	...
	public void update(IProgressMonitor monitor) {
		...
		try {
			...
			ISimpleTerm newSelection = findNewSelection(editor);
			Job job;
	...
	private ISimpleTerm findNewSelection(EditorState editor) {
		if (selection == null) return null;
		ISimpleTerm newAst = editor.getParseController().getCurrentAst();
