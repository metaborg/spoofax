...
public class {{javaName}}ParseControllerGenerated extends DynamicParseController {

  public static final String LANGUAGE = new String("{{name}}");
  ...
  protected static synchronized void setDescriptor(Descriptor descriptor) {
    {{javaName}}ParseControllerGenerated.descriptor = descriptor;
  }
  
  protected static void createDescriptor() {
      try {
        InputStream descriptorStream = {{javaName}}ParseControllerGenerated.class.getResourceAsStream(DESCRIPTOR);
        InputStream table = {{javaName}}ParseControllerGenerated.class.getResourceAsStream(TABLE);
        ...
        descriptor.setAttachmentProvider({{javaName}}ParseControllerGenerated.class);
  ...
  private static String getPluginLocation() {
      return {{javaName}}ParseController.class.getProtectionDomain().getCodeSource().getLocation().getFile();
  }
